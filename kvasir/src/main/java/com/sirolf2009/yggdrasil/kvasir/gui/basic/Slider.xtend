...
import com.sirolf2009.yggdrasil.kvasir.gui.Controller

@Accessors class Slider extends Controller {
	...
	override work() {
		if(mouseOver() && (mouseDragging() || mouseClicking())) {
			current = PApplet.map(applet.mouseX, x, x + getWidth(), min.floatValue, max.floatValue)
		}
		correctKnobPosition()
